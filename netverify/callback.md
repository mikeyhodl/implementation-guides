...
|lastNameAtBirth| |Last name at birth<br>• if idCountry = SLO |activation required |
|remarks| |Special remarks/titles<br>• if idCountry = SLO<br>Example: Mgr |activation required |
|disability| |Check if there is at least one disability icon available on the back side<br>• if idCountry = BHR<br>Possible values:<br />• YES<br />• NO |activation required |
